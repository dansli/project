...
and substr(extract_time,1,10) =
(SELECT substr(max(ZCALDAY),1,10) as ns_time
from NS.ZMM_DMV013)
GROUP BY  
 machine_id)  B
 ON A.machine_id=B.machine_id AND A.取数时间=B.extract_time)
 SELECT 
 *,CASE WHEN SKU宽度=0 THEN 0 ELSE  上机SKU宽度/SKU宽度 END AS 上机率
