with temp_oversea_detail as (
  ...
  CASE WHEN   CUSTOMER_2ND_CAT_NAME IN ('ROBOshop') then REGEXP_SUBSTR(customer_name, 'R[A-Z0-9]{2}-[A-Z0-9]{4}', 1, 1) else '' END AS 机器人数量,
	CASE WHEN   CUSTOMER_2ND_CAT_NAME IN ('ROBOshop') THEN PRODUCT_COUNT ELSE 0 END  AS 机器人销售商品总数量,
	CASE WHEN   CUSTOMER_2ND_CAT_NAME IN ('ROBOshop') THEN RMB_AMOUNT ELSE 0 END     AS 机器人销售商品总金额
from 
	ns.dws_oversea_order_v2_detail
WHERE 
 TRAN_DATE >=CURDATE() - INTERVAL 8 DAY
  AND TRAN_DATE  <=CURDATE() - INTERVAL 1 DAY
  AND SALE_COUNTRY IN  (select #机器人售卖的国家 top20商品按照数量排序 当地地区所有渠道的top20商品前7天#
  DISTINCT
	SALE_COUNTRY
from 
	ns.dws_oversea_order_v2_detail
...
SELECT A.国家地区,
  ...
  FROM  
JIQINEIBUKUCUN A 
  INNER  JOIN #当天库存最大的时间 机器内部库存情况#
 (SELECT 国家地区,
machine_id,MAX(取数时间 ) AS 最大时间 FROM  
JIQINEIBUKUCUN WHERE 
LEFT(取数时间,10)=CURDATE()
             GROUP  BY  
           国家地区  ,
machine_id  
             ) B 
  ON  A.国家地区=B.国家地区 AND A.machine_id=B.machine_id AND  A.取数时间=B.最大时间
...
from 
	temp_oversea_detail
  WHERE 销售商品总金额>0 AND 机器人销售商品总金额>0 #把购物袋这些没有金额的商品去掉#
  GROUP BY 
  SALE_COUNTRY,
	ITEM_CODE,
	ITEM_NAME
	...
	LEFT JOIN (
 SELECT #匹配top20商品在机器人内部有销售的机器人数量#
  国家地区,
  itemid,
