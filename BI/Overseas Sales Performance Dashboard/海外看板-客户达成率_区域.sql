...
base_sales AS (
  SELECT
    ...
    c.ec_platform,
    -- s.sale_country_name,
    round(s.incl_tax_rmb_amt,4) as incl_tax_rmb_amt
...
agg_sales AS (
  ...
  GROUP BY Region, Subsidiary, cus_id, cus_name,cus_code,cus_2nd_cat_name,ec_platform
  -- ,sale_country_name
)
...
SELECT
  ...
  sale.amt_last_year,
  -- 环比 MoM
  CASE WHEN amt_last_month != 0 THEN ROUND((Month_CNY_MTD - amt_last_month) / amt_last_month, 4) ELSE 0 END AS MOM,
  -- 同比 YoY
  CASE WHEN amt_last_year != 0 THEN ROUND((Month_CNY_MTD - amt_last_year) / amt_last_year, 4) ELSE 0 END AS YOY
...
and 1=1 ${if(cus_name == '',"","and   sale.cus_name in ('" + cus_name + "')")} 
