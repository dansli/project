...
left join goods as g on item.HD_ITEM_ID = g.code
  left join sds.ppro_series as s 
on g.series = s.code
left join ns.currencyrate as cr on sale.currency_id = cr.currency_id and cr.effectivedate = DATEADD(DAY, -DAY(sale.transaction_date), sale.transaction_date)
