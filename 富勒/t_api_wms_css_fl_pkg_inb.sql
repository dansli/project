-- --------------------------------------------- 
-- Subject:WMS_api_包裹信息(富勒-入库)
-- Author: 郭婧 
-- Create: 2025-06-10 
-- Update: 更新时间  姓名  更改内容
 -- ---------------------------------------------
-- create table dw.t_api_wms_css_fl_pkg_inb
-- (
...
--     primary key (id)
-- )comment'wms_api_包裹信息(富勒-入库)';

REPLACE INTO  dw.t_api_wms_css_fl_pkg_inb
SELECT  
    t1.id,
	...
	GROUP_CONCAT(distinct t1.exc_reason)  as exc_reason,	 -- 异常原因
	'FL' 		 			 	as wms_sys	 ,	    -- WMS系统(FL、TTX)
	'入库'				 		as order_type ,
	SYSDATE() AS etl_time ,-- 插入日期
	t1.create_time
FROM dw.t_api_wms_css_fl_goods_inb t1
LEFT JOIN 
 ( -- 视频信息
	 SELECT
		orderNO,
	    GROUP_CONCAT(distinct concat(originalFileName,'.mp4')) as video_file_name
		-- GROUP_CONCAT(concat('https://images-wms-oss-online-prd1.oss-cn-beijing.aliyuncs.com/FLUX_B2C/',originalFileName,'.mp4')) as video_file_name
	FROM
		fule.bsm_attachment
	WHERE
		organizationid = 'POP'
...
DELETE FROM dw.t_api_wms_css_fl_pkg_inb 
